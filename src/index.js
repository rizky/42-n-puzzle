...
const isCoordinateExist = (puzzle/* : Puzzle */) =>
  _.findIndex(steps, (step) => step === hash(puzzle)) !== -1;

const isCoordinateClosed = (puzzle/* : Puzzle */) =>
  _.findIndex(closedSteps, (step) => step === hash(puzzle)) !== -1;

const backtrack = async (puzzle/* : Puzzle */) => {
  if (hash(puzzle) === hash(goal)) { print_puzzle(puzzle); return; }
  if (isCoordinateExist(puzzle)) return;
  if (isCoordinateClosed(puzzle)) return;
